...
waldCI_constructor <- function(level, mean = NULL, sterr = NULL,
  ...
  return(new("wald_CI", level = level, mean = mean, sterr = sterr, 
             lb = lb, ub = ub))
}
...
setValidity("waldCI", function(object){
  ...
  if (object@level < 0) {
    return(paste("@level = ", object@level, "is smaller than 0, must be in (0,1)"))
  }
  if (object@level > 1) {
    return(paste("@level = ", object@level, "is greater than 1, must be in (0,1)"))
  }
  if (is.infinite(lb)) {
...
setMethod("transformCI", "waldCI", function(object, func){
  warning("Only monotonic functions make sense") #can i use this?
